...
3. 禁止混用图表工具（同一张图不混合 Mermaid 和 D2）
4. 禁止硬编码颜色（使用 `classes`/`classDef` 集中管理样式）
5. 禁止使用 HTML `<details>` 标签（用 Callout 折叠语法 `> [!type]-` 代替）
6. 禁止使用 HTML `<mark>` 标签（用 `==高亮==` 代替）
7. 禁止使用 HTML 注释 `<!-- -->`（用 `%%注释%%` 代替）
8. Callout 内嵌套代码块/表格时，每行必须以 `>` 开头
9. 表格内的管道符必须转义（`\|`）
10. YAML 值含 `:` 或 `#` 时必须用引号包裹
11. 禁止在同一段落混用 HTML 标签和 Markdown 格式（会导致格式失效）
12. YAML 中的内部链接必须用引号包裹（`key: "[[Link]]"`）
13. 脚注和任务列表不要放在 Callout 或表格内（不会渲染）

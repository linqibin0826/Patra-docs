...
## 与 react-learning-assistant Skill 的关系

本文件（CLAUDE.md）和 `react-learning-assistant` skill 分工明确：

| 文件 | 职责 | 内容 |
|------|------|------|
| **CLAUDE.md** | 定义"怎么教" | 教学理念、方法论、互动风格、反馈方式 |
| **Skill** | 定义"怎么做" | 文件创建流程、触发条件、格式规范 |
...
┌─────────────────────────────────────────────────────────────┐
│  阶段 1：上课                                                │
│  └─ 使用本文件的教学方法（苏格拉底提问、脚手架学习等）          │
├─────────────────────────────────────────────────────────────┤
│  阶段 2：出练习题  ← 学生说「出练习题」时触发 skill            │
│  └─ 根据上课 context 动态生成练习 + 测试                      │
├─────────────────────────────────────────────────────────────┤
│  阶段 3：批改讲解                                            │
│  └─ 使用本文件的引导方式（不直接给答案，成长型反馈）            │
├─────────────────────────────────────────────────────────────┤
│  阶段 4：生成笔记  ← 学生说「生成笔记」时触发 skill            │
│  └─ 结合学生理解情况生成个性化笔记                            │
├─────────────────────────────────────────────────────────────┤
│  阶段 5：收尾                                                │
│  └─ 更新 _MOC.md，本节课结束                                  │
...
> 这是**宏观流程中"阶段 1：上课"**的内部节奏，完整教学流程见 `react-learning-assistant` skill。

```
讲解概念 → 预测练习 → 代码演示 → 引导实践 → 苏格拉底式讨论
    ↑                                              │
    └──────────── 循环多个知识点 ←─────────────────┘
```
...
**每个知识点的教学节奏**：
1. **概念引入** - 这是什么？为什么重要？
2. **预测环节** - 展示代码，问"你觉得会输出什么？"
3. **代码演示** - 在 playground 创建可运行示例
4. **引导实践** - "运行 pnpm dev 看看效果"
5. **反思讨论** - "为什么这样设计？有什么替代方案？"
...
> 完整的收尾流程见 `react-learning-assistant` skill 的"阶段 4-5"。

当学生说「生成笔记」时，进入收尾阶段：
1. **生成个性化笔记** - 结合整节课的理解情况
2. **更新 _MOC.md** - 同步进度
3. **预告下次内容** - 建立学习期待
...
> 练习文件、测试文件、笔记文件的具体格式见 `react-learning-assistant` skill。

**核心原则**：
- 练习根据上课内容**动态生成**，不套用死板模板
- 笔记结合学生理解情况**个性化定制**
- 文档中不重复练习详情，只提供跳转链接

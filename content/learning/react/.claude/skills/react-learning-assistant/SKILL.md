...
> **禁止所有图都用 ASCII！** 必须根据内容特性选择最合适的工具。

### 选择决策表

| 内容类型 | 推荐工具 | 理由 |
|----------|----------|------|
| **流程/步骤** | Mermaid flowchart | 支持条件分支、循环标注 |
| **状态变化** | Mermaid stateDiagram | 清晰展示状态转换 |
| **时序交互** | Mermaid sequenceDiagram | 展示组件/函数调用顺序 |
| **组件层级/嵌套结构** | D2 | 支持容器嵌套、连接美观 |
| **架构图** | D2 | 复杂系统结构表达更清晰 |
| **数据对比/统计** | Charts | 柱状图、折线图、饼图 |
| **简单概念对比** | ASCII | 两三个元素的简单对比 |
| **内存布局/栈堆** | ASCII | 传统表达方式，直观 |
